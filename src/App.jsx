...
const ColorPlate = ({color, index, onIconClick, lockStatus}) => {
  ...
  return (
    ...
    >
      <p onClick={handleCopy} className="p-2 hover:cursor-pointer mix-blend-overlay">{ color }</p>
      <div className="cursor-pointer mix-blend-overlay" onClick={handleLockUnlock}>
        { lockStatus ? 
  ...
  return (
    <div className="flex flex-col h-screen">
      ...
      <div className="h-24 flex items-center justify-between px-6 md:px-8 lg:px-12">
        ...
        <h2 className='text-4l text-black-300'>Hit the space bar to generate a new color palette.</h2>
        <button onClick={handleDownload} className="bg-red-500 px-3 py-2 rounded hover:bg-red-600 text-white">Download</button>
      </div>
